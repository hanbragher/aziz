...
@section('content')
    <div class="fixed-action-btn ">
        <a class="btn-floating btn-large red" href="{{route('announcements.create')}}">
    ...
    <div class="row">
        <div class="col s12">
            ...
            @include('inc.middlemenu', ['avatar'=>$announcement->image, 'header'=> '<a href="'.route('announcements.show', $announcement->id).'" target="blank"><i class="material-icons">open_in_new</i></a>' ])
        </div>
    </div>
        ...
        <div class="col s12 m12 l8">
            <div class="row">
                ...
                <div id="image_delete" class="modal delete">
                    <form action="{{route('announcements.update', $announcement->id)}}" method="post" id="form1" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="modal-content">
                            ...
                            <input id="announcement_id" name="announcement_id" value="" type="hidden">
                ...
                <div id="add_title" class="modal title">
                    <form action="{{route('announcements.update', $announcement->id)}}" method="post" id="form1" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                ...
                <form action="{{route('announcements.update', $announcement->id)}}" method="post" id="form" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="row">
                        <div class="input-field col s4">
                            <input value='{{$announcement->title}}' id="input_text" type="text" data-length="100" name="title" required>
                    ...
                    <div class="row">
                        ...
                        <div class="file-field input-field col s7">
                            @if(12-($images_count = $announcement->images->count()) > 0)
                            ...
                            @foreach($announcement->images as $image)
                                <div class="col col s6 m3 l2">
                                    <div class="card">
                                        ...
                                        <a data-imageid='{{$image->id}}' data-announcementid='{{$announcement->id}}' data-imagetitle='{{$image->title}}' href="#add_title" class="modal-open-title truncate grey-text center" ><i class="material-icons tiny">create</i>{{!empty($image->title)?$image->title:'Add title'}}</a>

                                        <p><a class="teal-text" href="{{$image->file}}" download><i class="material-icons">file_download</i></a></p>
                                        <a data-imageid='{{$image->id}}' data-announcementid='{{$announcement->id}}' href="#delete_image" class="modal-open-delete btn-floating halfway-fab waves-effect waves-light red" ><i class="material-icons">delete_forever</i></a>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="row">
                        <div class="input-field col s12">
                            <textarea  id="textarea1" class="materialize-textarea" name="text" required>{{$announcement->text}}</textarea>
